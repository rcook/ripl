 ...
 *
 *		HAAR WAVELET EXPERIMENT NO. 2
 *		Two-dimensional Haar wavelet transform example. Demonstration of
 *		reconstruction of a 2D data set.
 *
 *		Ripl Version 1.1, last update: 23 February 1998.
 *
 *		History:
 *			23/2/98:		'ftHaar' renamed 'ftHaar2'.
 *			31/1/98:		first version.
 *
...
int main(int argc, char **argv) {
    ...
    if (argc<2) {
        riplMessage(itInfo, "Usage: haar2 <filename> <threshold>.\n"
            "Outputs reconstruction of Gaussian surface after\n"
            "thresholding Haar wavelet coefficients as\n"
            "specified.\n");
        return EXIT_FAILURE;
    }
    if (!riplArgGet_float(argv[2], &threshold)) {
        riplMessage(itError, "Usage: haar2 <filename> <threshold>.\n");
        return EXIT_FAILURE;
    }
    ...
    if (!pfile) {
        riplMessage(itError, "haar2: unable to open %s for output.\n",
            argv[1]);
        return EXIT_FAILURE;
    }
    ...
    fprintf(pfile, "HAAR2, 31/1/98.\n"
        "Approximation to Gaussian surface using Haar wavelets.\n"
        "Threshold = %e.\n"
